...
include(__DIR__."/app/vergi_kate.php");
$PageTitle=$W92_PageTitle;
include("prc/top.php");
?>
<!-- datatables -->
  ...
  <section>
    <div class="container-xxl mb-5">
      <div class="table_body_bg">
        <h4 class="d-flex justify-content-between w-100"><span class="pe-3"><?=$W92_Text1?></span> <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addmodal"><i class="fa-solid fa-plus pe-1"></i><?=$W92_Text2?></button></h4>
        <hr>
          ...
          <tbody>
            <?php foreach ($DataList as $key => $value) {?>
              <tr id="<?=$DataList[$key]["KateID"]?>">
                ...
                <td>
                 <button type="button" class="btn btn-primary btn-sm me-1 row_info" data-id='<?=$DataList[$key]["KateID"]?>' data-bs-toggle="modal" data-bs-target="#editmodal"><i class="fa-regular fa-pen-to-square"></i> <?=$W92_Text6?></button>
                 <button type="button" class="btn btn-danger btn-sm sil"  data-id='<?=$DataList[$key]["KateID"]?>'><i class="fa-regular fa-trash-can"></i> <?=$W92_Text7?></button>
               </td>
             </tr>
           <?php }  ?>
         </tbody>
    ...
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="addmodalLabel"><?=$W92_Text8?></h1>
          ...
          <form class="w-100 form_send" action="" method="post">
           <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
             <div class="form-floating">
              <input type="text" class="form-control" id="name" placeholder="<?=$W92_Text9?>" name="name">
              <label for="name"><?=$W92_Text9?></label>
            </div>
          </div>
          <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
            <div class="form-floating">
              <textarea class="form-control textarea100" placeholder="Açıklama" id="desc" name="desc"></textarea>
              <label for="desc">Açıklama</label>
            </div>
          </div>
          <input type="hidden" name="form_add">
          <div class="btn_col"><button type="submit" class="btn_style1"><i class="fa-solid fa-plus">&nbsp;</i><?=$W92_Text10?></button> </div>
        </form>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="editmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="editmodalLabel"><?=$W92_Text11?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        ...
        <div class="btn_col"><button type="submit" class="btn_style1"><i class="fa-solid fa-rotate">&nbsp;</i><?=$W92_Text12?></button> </div>
      </form>
    </div>
  </div>
</div>
...
<script>
  ...
  $("body").on("click",".row_info",function() {
    ...
    $.ajax({
      ...
      if (obj.vergituru){$('#guncelleme input:radio[name="vergituru"]').filter('[value="1"]').attr('checked', true);}
      else{$('#guncelleme input:radio[name="vergituru"]').filter('[value="0"]').attr('checked', true);}                       
