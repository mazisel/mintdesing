...
include(__DIR__."/app/city.php");
$PageTitle=$W84_PageTtitle;
include("prc/top.php");
?>
<!-- datatables -->
    ...
    <div class="container-xxl mb-5">
      <div class="table_body_bg">
        <h4 class="d-flex justify-content-between w-100">
          <span class="pe-3"><?=$W84_Text1?></span> 
          <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addmodal"><i class="fa-solid fa-plus pe-1"></i><?=$W84_Text2?></button>
        </h4>
        <hr>
        <table id="devnanotek_table" class="table table-striped display responsive nowrap pt-2">
          <thead>
            <tr>
              <th><?=$W84_Text3?></th>
              <th><?=$W84_Text4?></th>
              <th width="10%"><?=$W84_Text5?></th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($DataList as $key => $value) {?>
              <tr id="<?=$DataList[$key]["CityID"]?>">
                <td> <?=$DataList[$key]["CityPostaKodu"] ?> </td>
               <td> <?=$DataList[$key]["CityName"] ?> </td>
               <td>
                 <button type="button" class="btn btn-primary btn-sm me-1 row_info" data-id='<?=$DataList[$key]["CityID"]?>' data-bs-toggle="modal" data-bs-target="#editmodal"><i class="fa-regular fa-pen-to-square"></i> <?=$W84_Text6?></button>
                 <button type="button" class="btn btn-danger btn-sm sil"  data-id='<?=$DataList[$key]["CityID"]?>'><i class="fa-regular fa-trash-can"></i> <?=$W84_Text7?></button>
               </td>
             </tr>
           <?php }  ?>
         </tbody>
    ...
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="addmodalLabel"><?=$W84_Text8?></h1>
          ...
          <form class="w-100 form_send" action="" method="post">
           <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
             <div class="form-floating">
              <input type="text" class="form-control" id="alankodu" placeholder="<?=$W84_Text9?>" name="alankodu">
              <label for="alankodu"><?=$W84_Text9?></label>
            </div>
          </div>
          <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
           <div class="form-floating">
            <input type="text" class="form-control" id="name" placeholder="<?=$W84_Text10?>" name="name">
            <label for="name"><?=$W84_Text10?></label>
          </div>
        </div>
        <input type="hidden" name="form_add">
        <div class="btn_col"><button type="submit" class="btn_style1"><i class="fa-solid fa-plus">&nbsp;</i><?=$W84_Text11?></button> </div>
      </form>
    </div>
  </div>
</div>
...
<div class="modal fade" id="editmodal" tabindex="-1" aria-labelledby="editmodalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="editmodalLabel"><?=$W84_Text12?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
       ...
       <div class="btn_col"><button type="submit" class="btn_style1"><i class="fa-solid fa-rotate">&nbsp;</i><?=$W84_Text13?></button> </div>
     </form>
   </div>
 </div>
</div>
