...
function MailGonder($Baslik,$Mesaj,$Alicilar=[],$YonetimAlicilari=0,$Options=[])
{
	global $db;
	$smtp_ayar=$db->prepare("SELECT * FROM smtp_ayar WHERE SmtpID =?");
	$smtp_ayar->execute(array(1));
	...
	$konu=$Baslik;
	$icerik=$Mesaj;		
	if($Options['SmtpGondericiAdi']){
		$gonderen_Adi = $Options['SmtpGondericiAdi'];
	}
	$mail = new PHPMailer(true);
	try {
	    //Server settings
	    ...
	    */
	    ...
	    if(is_array($Options['Files']) && count($Options['Files'])){
	    	foreach ($Options['Files'] as $pathfile) {
...
function mailSend($Options=[])
{
	global $db,$DNCrud,$Firma;

	if ($Options['SmtpID']) {
		$sql=$DNCrud->ReadData("firma_smtp",["sql"=>"WHERE FirmaID={$Firma['FirmaID']} AND SmtpID={$Options['SmtpID']}"]); 
    	$smtpRow=$sql->fetch(PDO::FETCH_ASSOC);
    	$gonderen_host=$smtpRow['SmtpSunucu'];
		$gonderen_mail=$smtpRow['SmtpEmail'];
		$gonderen_mail_sifre=$smtpRow['SmtpPass'];
		$gonderen_Adi=$smtpRow['SmtpGondericiAd'];
		$port=$smtpRow['SmtpProt'];
		$siteUrl=$_SERVER['HTTP_HOST'];	
		$AliciMailler=explode(',', $smtpRow['SmtpAlicilar']); 
	}else{
		$smtp_ayar=$db->prepare("SELECT * FROM smtp_ayar WHERE SmtpID =?");
		$smtp_ayar->execute([1]);
		$smtp_cek=$smtp_ayar->fetch(PDO::FETCH_ASSOC);
	...
	//$Options['Files']
	//$Options['SmtpGondericiAdi']
	$Baslik=$Options['title'];//text
	$Mesaj=$Options['content'];//text
	if(isset($Options['eMails'])){
		$Alicilar=$Options['eMails'];//arry
	}else{$Alicilar=[];}	
	$YonetimAlicilari=$Options['adminSend'];//number
